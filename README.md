...
- Can have a library, an executable and a test suite, via [summoner](https://github.com/kowainik/summoner)
- Comes with a shell.nix which provides an environment with ghcid and ormolu;
- Manages the build environment with [lorri](https://github.com/target/lorri) and [direnv](https://direnv.net/);
- `ghci` integrates with Haddock and Hoogle for all dependencies (`:doc`, `:hoogle`);
- Can be built into a Docker container;
- Uses a pinned nixpkgs managed by [niv](https://github.com/nmattia/niv).;
